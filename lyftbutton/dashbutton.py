...
def _from_dynamo(serial_number=None, lyft_id=None):
    ...
    if not serial_number:
        items = table.query(
            ...
            ExpressionAttributeValues={":lyft_id": lyft_id},
        ).get("Items")
        if items:
...
class DashButton:
    ...
    def _set_lyft_account(self, lyft_account):
        _to_dynamo(self.serial_number, "lyft_id", lyft_account.id)
        _to_dynamo(
            self.serial_number, "lyft_credentials", lyft_account.credentials
        )
    ...
    def _get_location(self, field):
        ...
        if location_data:
            location_data = {k: float(v) for k, v in location_data.items()}
        return Location(**location_data) if location_data else None

    def _set_location(self, field, location):
        location_data = {
            k: Decimal("%.4f" % v) for k, v in attr.asdict(location).items()
        }
